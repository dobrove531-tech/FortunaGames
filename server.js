...
let achievementsList = [
  ...
  { id: "prestige_1", name: "Перерождение", target: 1, reward: 50000 }
];
...
function runLottery() {
  if (Date.now() - lastLotteryTime < LOTTERY_INTERVAL) return;
...
function checkAchievements(user) {
  ...
  for (let ach of achievementsList) {
    ...
    if (ach.id === "prestige_1" && user.prestige >= 1) done = true;
    if (done) {
...
app.post('/api/login', (req, res) => {
  ...
  if (user) {
    res.json({ success: true, user: { 
      ...
      achievements: user.achievements, stats: user.stats, lastWheel: user.lastWheel,
...
app.post('/api/register', (req, res) => {
  ...
  users.push({
    ...
    achievements: [], lastWheel: 0, lotteryTickets: 0,
    stats: { totalEarned: 0, totalSpent: 0, lotteryWins: 0, giftsSent: 0 }
  });
  saveData();
...
app.post('/api/admin/set-level', (req, res) => {
  ...
  if (!admin || admin.role !== "admin") return res.json({ success: false, message: "Нет прав" });
...
app.post('/api/admin/give-item', (req, res) => {
  const { adminId, userId, itemId, itemName } = req.body;
  const admin = users.find(u => u.id === adminId);
  const user = users.find(u => u.id === userId);
  if (!admin || admin.role !== "admin") return res.json({ success: false, message: "Нет прав" });
  if (!user) return res.json({ success: false, message: "Пользователь не найден" });
  if (!user.inventory) user.inventory = [];
  user.inventory.push({ id: itemId, name: itemName, type: "admin" });
  saveData();
  res.json({ success: true, message: `Выдан предмет ${itemName}` });
});
